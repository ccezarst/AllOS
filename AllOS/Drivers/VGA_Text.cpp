...
union VgaCharacter{
	struct  __attribute__((packed)){
		...
		unsigned int foregroundCol:4;
		unsigned int backgroundCol:3;
		unsigned int isBlinking:1;
	}Character;
	uint8_t CharacterBytes[2];
};

void kprintCol(const char* s, int col, bool blinking){		//Print: with colours!
  uint8_t* charPtr = (uint8_t*)s;
  uint16_t i = CursorPos;
  while(*charPtr != 0)
  {
	switch (*charPtr) {
	  ...
	  default:
		...
		// 	trueBit urmom;
		// 	urmom.abcdd = 1;
		// 	*(VIDEO_MEMORY + i * 2 + 8) = urmom.abcdd; 
	  	// }
	//   *(VIDEO_MEMORY + i*2) = *charPtr;
	//   *(VIDEO_MEMORY + i*2 + 1) = col;
			VgaCharacter character;
			character.Character.character = *charPtr;
			character.Character.foregroundCol = col;
			character.Character.backgroundCol = 0;
			if(blinking){
				character.Character.isBlinking = 1;
			}else{
				character.Character.isBlinking = 0;
			}
			
			*(VIDEO_MEMORY + i*2) = character.CharacterBytes[0];
			*(VIDEO_MEMORY + i*2 + 1) = character.CharacterBytes[1];
	  i++;
	}
...
void kprintCharCol(const char* s, int col, bool blinking){		//Print: with colours!
  uint8_t* charPtr = (uint8_t*)s;
  uint16_t i = CursorPos;
	switch (*charPtr) {
		case 10:	
...
void printError(const char* s){
    kprintCol(s, ERROR_COLOR, false);
}
void deleteChar(){
	MoveCursorLR(-1);
	VgaCharacter nulle;
	nulle.Character.character = 'a';
	nulle.Character.foregroundCol = 0;
	nulle.Character.backgroundCol = 0;
	nulle.Character.isBlinking = 0;
	*(VIDEO_MEMORY + CursorPos * 2) = nulle.CharacterBytes[0];
	*(VIDEO_MEMORY + CursorPos * 2 + 1) = nulle.CharacterBytes[1];
